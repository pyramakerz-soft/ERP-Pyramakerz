...
export class MedalComponent {
  ...
  validationErrors: { [key in keyof Medal]?: string } = {};
  isLoading = false;
  ...
  Edit(row: Medal) {
    this.mode = 'Edit';
    this.medalServ.GetByID(row.id, this.DomainName).subscribe((d) => {
      this.medal = d;
    });
    this.openModal();
  }
  ...
  isFormValid(): boolean {
    let isValid = true;
    for (const key in this.medal) {
      if (this.medal.hasOwnProperty(key)) {
        const field = key as keyof Medal;
        if (!this.medal[field]) {
          if (
            field == 'arabicName' ||
  ...
  capitalizeField(field: keyof Medal): string {
    return field.charAt(0).toUpperCase() + field.slice(1).replace(/_/g, ' ');
  }

  onInputValueChange(event: { field: keyof Medal; value: any }) {
    const { field, value } = event;
    (this.medal as any)[field] = value;
    if (value) {
  ...
  async onSearchEvent(event: { key: string; value: any }) {
    ...
    try {
      const data: Medal[] = await firstValueFrom(
