...
export class PerformanceTypeComponent {
  ...
  validationErrors: { [key in keyof PerformanceType]?: string } = {};
  isLoading = false;
  ...
  constructor(
    ...
    public TypeServ: PerformanceTypeService,
  ...
  isFormValid(): boolean {
    ...
    for (const key in this.Type) {
      if (this.Type.hasOwnProperty(key)) {
        const field = key as keyof PerformanceType;
        if (!this.Type[field]) {
          if (
            field == 'englishName' ||
            field == 'arabicName'
          ) {
            const displayName = field === 'englishName' ? 'English Name' : 'Arabic Name';
  ...
  capitalizeField(field: keyof PerformanceType): string {
    return field.charAt(0).toUpperCase() + field.slice(1).replace(/_/g, ' ');
  }

  onInputValueChange(event: { field: keyof PerformanceType; value: any }) {
  ...
  async onSearchEvent(event: { key: string; value: any }) {
    ...
    try {
      const data: PerformanceType[] = await firstValueFrom(
