...
export class SectionComponent {
  ...
  async onSearchEvent(event: { key: string; value: any }) {
    ...
    try {
      const data: Section[] = await firstValueFrom(
        this.sectionService.Get(this.DomainName)
      );
  ...
  getSectionData() {
    this.sectionData = [];
    this.sectionService.Get(this.DomainName).subscribe((data) => {
      this.sectionData = data;
    });
  }

  getSectionById(id: number) {
    this.sectionService.GetByID(id, this.DomainName).subscribe((data) => {
      this.section = data;
    });
  }
