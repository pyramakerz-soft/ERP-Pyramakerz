...
export class LessonResourcesTypeComponent {
 User_Data_After_Login: TokenData = new TokenData('', 0, 0, 0, 0, '', '', '', '', '');
  ...
  Type: LessonResourceType = new LessonResourceType();

  validationErrors: { [key in keyof LessonResourceType]?: string } = {};
  isLoading = false;
  ...
  Create() {
    this.mode = 'Create';
    this.Type = new LessonResourceType();
  ...
  isFormValid(): boolean {
    ...
    for (const key in this.Type) {
      if (this.Type.hasOwnProperty(key)) {
        const field = key as keyof LessonResourceType;
        if (!this.Type[field]) {
          if (
            field == 'englishName' ||
            field == 'arabicName'
  ...
  capitalizeField(field: keyof LessonResourceType): string {
    return field.charAt(0).toUpperCase() + field.slice(1).replace(/_/g, ' ');
  }

  onInputValueChange(event: { field: keyof LessonResourceType; value: any }) {
  ...
  async onSearchEvent(event: { key: string; value: any }) {
    ...
    try {
      const data: LessonResourceType[] = await firstValueFrom(
