...
export class MedicalReportComponent implements OnInit {
  ...
  async viewReport() {
    if (!this.selectedSchool || !this.selectedGrade || !this.selectedClass) {
      Swal.fire({
        ...
        text: 'Please select School, Grade and Class',
    ...
    try {
      ...
      switch (this.selectedTab) {
        ...
        case 'MH By Doctor':
          const service =
            this.selectedTab === 'MH By Parent'
              ? this.medicalReportService.getAllMHByParent
              : this.medicalHistoryService.GetByDoctor;
        ...
        case 'Hygiene Form':
          ...
          this.tableData = data.flatMap((form) =>
            form.studentHygieneTypes.map((student: any) => ({
              ...
              types:
                student.hygieneTypeId === 1
                  ? 'Type 1'
                  : student.hygieneTypeId === 2
                  ? 'Type 2'
                  : 'N/A',
              comment: student.comment || 'None',
              actionTaken: student.actionTaken || 'None',
            }))
          );
          break;
  ...
  getCurrentHeaders(): string[] {
    switch (this.selectedTab) {
      ...
      case 'Hygiene Form':
        return ['Date', 'Attendance', 'Type', 'Comment', 'Action Taken'];
      case 'Follow Up':
        return ['Date', 'Diagnosis', 'Drug', 'Dose', 'Recommendations'];
      default:
        return [];
    }
  }
