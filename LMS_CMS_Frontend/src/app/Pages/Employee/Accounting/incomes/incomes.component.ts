...
export class IncomesComponent {
 ...
 isRtl: boolean = false;
  ...
  Delete(id: number) {
     Swal.fire({
      title: this.translate.instant('Are you sure you want to') + " " + this.translate.instant('delete')+ " " + this.translate.instant('هذه') + " " + this.translate.instant('Income') + this.translate.instant('?'),
      icon: 'warning',
  ...
  onInputValueChange(event: { field: keyof Income; value: any }) {
    const { field, value } = event;
    (this.income as any)[field] = value;
    if (value) {
  ...
  async onSearchEvent(event: { key: string; value: any }) {
    ...
    try {
      const data: Income[] = await firstValueFrom(
        this.IncomeServ.Get(this.DomainName)
      );
      this.TableData = data || [];
