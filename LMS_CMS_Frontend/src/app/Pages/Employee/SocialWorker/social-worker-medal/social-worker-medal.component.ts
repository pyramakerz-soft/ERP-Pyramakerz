...
export class SocialWorkerMedalComponent {
  ...
  socialWorkerMedal: SocialWorkerMedal = new SocialWorkerMedal();

  validationErrors: { [key in keyof SocialWorkerMedal]?: string } = {};
  isLoading = false;
  ...
  Create() {
    this.mode = 'Create';
    this.socialWorkerMedal = new SocialWorkerMedal();
  ...
  Edit(row: SocialWorkerMedal) {
    this.mode = 'Edit';
    this.SocialWorkerMedalServ.GetByID(row.id, this.DomainName).subscribe((d) => {
      this.socialWorkerMedal = d;
    });
    this.openModal();
  }
  ...
  CreateOREdit() {
    if (this.isFormValid()) {
      this.isLoading = true;
      if (this.mode == 'Create') {
        this.SocialWorkerMedalServ.Add(
      ...
      if (this.mode == 'Edit') {
        this.SocialWorkerMedalServ.Edit(
          this.socialWorkerMedal,
          this.DomainName
        ).subscribe(
          (d) => {
  ...
  isFormValid(): boolean {
    let isValid = true;
    for (const key in this.socialWorkerMedal) {
      if (this.socialWorkerMedal.hasOwnProperty(key)) {
        const field = key as keyof SocialWorkerMedal;
  ...
  capitalizeField(field: keyof SocialWorkerMedal): string {
    return field.charAt(0).toUpperCase() + field.slice(1).replace(/_/g, ' ');
  }

  onInputValueChange(event: { field: keyof SocialWorkerMedal; value: any }) {
    const { field, value } = event;
    (this.socialWorkerMedal as any)[field] = value;
    if (value) {
  ...
  async onSearchEvent(event: { key: string; value: any }) {
    ...
    try {
      const data: SocialWorkerMedal[] = await firstValueFrom(
        this.SocialWorkerMedalServ.Get(this.DomainName)
      );
      this.TableData = data || [];
