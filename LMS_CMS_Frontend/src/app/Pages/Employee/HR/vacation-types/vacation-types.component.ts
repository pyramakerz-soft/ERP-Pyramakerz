...
export class VacationTypesComponent {
  ...
  vacationType: VacationTypes = new VacationTypes();

  validationErrors: { [key in keyof VacationTypes]?: string } = {};
  isLoading = false;
  ...
  Create() {
    this.mode = 'Create';
    this.vacationType = new VacationTypes();
  ...
  Edit(id: number) {
    this.mode = 'Edit';
    this.VacationTypesServ.GetByID(id, this.DomainName).subscribe((d) => {
      this.vacationType = d;
    });
    this.openModal();
  }
  ...
  onInputValueChange(event: { field: keyof VacationTypes; value: any }) {
    const { field, value } = event;
    (this.vacationType as any)[field] = value;
    if (value) {
  ...
  async onSearchEvent(event: { key: string; value: any }) {
    ...
    try {
      const data: VacationTypes[] = await firstValueFrom(
        this.VacationTypesServ.Get(this.DomainName)
      );
      this.TableData = data || [];
