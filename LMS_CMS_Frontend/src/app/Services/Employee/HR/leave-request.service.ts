...
export class LeaveRequestService {
  ...
  GetRemainLeavRequestsByEmployeeId(Eid: number, date: string, DomainName: string) {
    ...
    const headers = new HttpHeaders()
      ...
      .set('Authorization', `Bearer ${token}`)
      .set('Content-Type', 'application/json');
    return this.http.get<Employee>(`${this.baseUrl}/LeaveRequest/getRemainLeavRequestsByEmployeeId/${Eid}/${date}`, { headers })
  }
  ...
  Add(LeaveRequest: LeaveRequest, DomainName: string) {
    ...
    return this.http.post<any>(`${this.baseUrl}/LeaveRequest`, LeaveRequest, { headers, responseType: 'text' as 'json' });
  }

  Edit(LeaveRequest: LeaveRequest, DomainName: string) {
    if (DomainName != null) {
      this.header = DomainName
    }
    const token = localStorage.getItem("current_token");
    const headers = new HttpHeaders()
      ...
      .set('Authorization', `Bearer ${token}`)
      .set('Content-Type', 'application/json');
    return this.http.put<LeaveRequest>(`${this.baseUrl}/LeaveRequest`, LeaveRequest, { headers });
  }
