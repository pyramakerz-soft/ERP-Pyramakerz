...
export class AppointmentService {
  ...
  GetBySchoolIdWithPaggination(SchoolId: number, DomainName: string, pageNumber: number, pageSize: number) {
    ...
    const headers = new HttpHeaders()
      ...
      .set('Authorization', `Bearer ${token}`)
      .set('Content-Type', 'application/json');
    return this.http.get<{ data: Appointment[], pagination: any }>(
      `${this.baseUrl}/Appointment/BySchoolIdWithPaggination/${SchoolId}?pageNumber=${pageNumber}&pageSize=${pageSize}`,
      { headers }
    );
  }  
