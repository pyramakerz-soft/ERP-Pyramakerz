...
export class AccountingTreeChartService {
  ...
  GetByLinkFileId(linkFileID:number, DomainName:string) {
     if(DomainName!=null) {
      this.header=DomainName 
    }
    const token = localStorage.getItem("current_token");
    const headers = new HttpHeaders()
      ...
      .set('Authorization', `Bearer ${token}`)
      .set('Content-Type', 'application/json');
    return this.http.get<AccountingTreeChart[]>(`${this.baseUrl}/AccountingTreeChart/GetByLinkFileId/${linkFileID}`, { headers })
  }
  ...
  GetMainDataChildFiltered(id: number, DomainName: string) {
    ...
    const headers = new HttpHeaders()
      ...
      .set('Authorization', `Bearer ${token}`)
      .set('Content-Type', 'application/json');
    return this.http.get<AccountingTreeChart[]>(`${this.baseUrl}/AccountingTreeChart/GetMainDataChildFiltered/${id}`, { headers })
  }
