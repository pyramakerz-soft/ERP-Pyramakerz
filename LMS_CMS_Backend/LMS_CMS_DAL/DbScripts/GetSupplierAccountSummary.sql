...
GO
CREATE OR ALTER PROCEDURE [dbo].[GetSupplierAccountSummary]
    @DateFrom DATE = NULL,
    @DateTo DATE = NULL,
    @MainAccNo BIGINT = 0,
    @SubAccNo BIGINT = 0,
...
BEGIN
    ...
    BEGIN
        ...
        LEFT JOIN dbo.EntriesFun(@DateFrom, @DateTo, @MainAccNo, @SubAccNo, @linkFileID) E
            ON E.LinkFileID = 2 AND (E.SubAccountNo = S.ID OR E.SubAccountNo = 0)
        WHERE (@SubAccNo = 0 OR S.ID = @SubAccNo)
    ...
    BEGIN
        ...
        FROM Saves SF
        LEFT JOIN dbo.EntriesFun(@DateFrom, @DateTo, @MainAccNo, @SubAccNo, @linkFileID) E
            ON E.LinkFileID = 5 AND (E.SubAccountNo = SF.ID OR E.SubAccountNo = 0)
        WHERE (@SubAccNo = 0 OR SF.ID = @SubAccNo)
