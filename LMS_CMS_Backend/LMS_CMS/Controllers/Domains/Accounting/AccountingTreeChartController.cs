...
namespace LMS_CMS_PL.Controllers.Domains.Accounting
{
    ...
    public class AccountingTreeChartController : ControllerBase
    {
        ...
        public AccountingTreeChartController(DbContextFactoryService dbContextFactory, IMapper mapper, CheckPageAccessService checkPageAccessService)
        {
        ...
        [HttpGet("GetByLinkFileId/{linkFileID}")]
        [Authorize_Endpoint_(
            allowedTypes: new[] { "octa", "employee" },
            pages: new[] { "Accounting" }
        )]
        public async Task<IActionResult> GetByLinkFileId(long linkFileID)
        {
            UOW Unit_Of_Work = _dbContextFactory.CreateOneDbContext(HttpContext);

            List<AccountingTreeChart> accountingTreeCharts = await Unit_Of_Work.accountingTreeChart_Repository.Select_All_With_IncludesById<AccountingTreeChart>(
                    f => f.IsDeleted != true && f.LinkFileID == linkFileID,
                    query => query.Include(ac => ac.LinkFile),
                    query => query.Include(ac => ac.MotionType),
                    query => query.Include(ac => ac.SubType),
                    query => query.Include(ac => ac.EndType),
        ...
        public async Task<IActionResult> Edit(AccountingTreeChartAddDTO EditedAccountingTreeChart)
        {
            ...
            {
                IActionResult? accessCheck = _checkPageAccessService.CheckIfEditPageAvailable(Unit_Of_Work, "Accounting", roleId, userId, accountExists);
                if (accessCheck != null)
                {
                    return accessCheck;
                }
