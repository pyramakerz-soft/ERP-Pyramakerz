        ...
        {  
            ...
            if(userTypeID == 1) // employee
            {
                if(filterList != null)
                {
                    ...
                    } else if (filterList.DepartmentID != null)
                    {
                    ...
                    } else if (filterList.NotAssignedToDepartment == true)
                    {
                        List<Employee> employees = Unit_Of_Work.employee_Repository.FindBy(d => d.IsDeleted != true && d.DepartmentID == null);
                        if (employees == null || employees.Count == 0)
                        {
                            return new List<long>();
                        }
            ...
            } else if (userTypeID == 3) // parent
            {
                if (filterList != null)
                {
                    if (filterList.ParentID != null)
                    {
                        Parent parent = Unit_Of_Work.parent_Repository.First_Or_Default(e => !e.IsDeleted != true && e.ID == filterList.ParentID);

                        if (parent == null)
                        {
                            throw new Exception("No Parent with this ID");
                        }
                        return new List<long> { parent.ID };
                    } 
                    else
                    {
                        List<Parent> parents = Unit_Of_Work.parent_Repository.FindBy(d => d.IsDeleted != true);
                        return parents.Select(e => e.ID).ToList();
                    }
                }
                else
                {
